...
from torch.distributed.fsdp import (
    ...
    StateDictType,
    FullStateDictConfig, # general model non-sharded, non-flatten params
    LocalStateDictConfig, # flattened params, usable only by FSDP
    # ShardedStateDictConfig, # un-flattened param but shards, usuable by other parrallel schemes
)

from torch.distributed._shard.checkpoint import (
    FileSystemReader,
    FileSystemWriter,
    save, # save_state_dict 버전 변경 됨
    load, # load_state_dict 버전 변경 됨
)
...
def get_date_of_run():
    ...
    example: 2024-10-28-15:05:12_PM
    """
...
def load_model_sharded(model, rank, cfg):
    ...
    if rank == 0:
        print(f"Sharded state checkpoint loaded fro {load_dir}")
...
def save_model_and_optimizer_sharded(model, rank, cfg, optim=None):
    ...
    if rank == 0:
        print(f"Shardded state checkpoint saved to {save_dir}")
        print(
...
def load_optimizer_checkpoint(model, optimizer_checkpoint_path, rank):
    ...
    if not optimizer_checkpoint_path.is_file():
        print(
            f"warning - optimizer checkpoint is not present {optimizer_checkpoint_path}. Returning."
        )
        return
    ...
    # rank0에서 불러온 OSD를 모든 다른 프로세스에 분산시킴. 이를 통해 각 프로세스는 자신이 관리하는 모델 파라미터에 해당하는 옵티마이저 상태만을 갖게 됨.
    # called from all ranks, though only rank0 has a valid param for full_osd
    shared_osd = FSDP.scatter_full_optim_state_dict(full_osd, model)
